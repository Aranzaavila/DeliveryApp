...
class DeliveryApp(ctk.CTk):
    def __init__(self, controller: AppController):
        ...
        self.btn_dashboard = ctk.CTkButton(self.sidebar, text="🏠 Dashboard", command=self.show_dashboard_view)
        self.btn_dashboard.grid(row=1, column=0, sticky="ew", padx=20, pady=10)

        self.btn_create_delivery = ctk.CTkButton(self.sidebar, text="➕ Create Delivery", command=self.show_create_delivery_view)
        self.btn_create_delivery.grid(row=2, column=0, sticky="ew", padx=20, pady=10)

        self.btn_view_deliveries = ctk.CTkButton(self.sidebar, text="🚚 Deliveries", command=self.show_view_deliveries_view)
        self.btn_view_deliveries.grid(row=3, column=0, sticky="ew", padx=20, pady=10)

        self.btn_view_invoices = ctk.CTkButton(self.sidebar, text="💸 Invoice", command=self.show_invoices_view)
        self.btn_view_invoices.grid(row=4, column=0, sticky="ew", padx=20, pady=10)
        
        self.btn_reminders = ctk.CTkButton(self.sidebar, text="🔔 Reminders", command=self.check_reminders)
        ...
        # --- Instanciamos cada vista y la guardamos ---
        for F in (DashboardFrame, CreateDeliveryFrame, ViewDeliveriesFrame, InvoiceFrame):
