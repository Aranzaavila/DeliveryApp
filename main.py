...
def save_to_db(conn, freelancer):
    ...
    cursor.execute( """ INSERT OR REPLACE INTO freelancers (freelancer_id, user_id, name, email) VALUE (?, ?, ?, ?)
...
def load_from_db(conn, freelancer_id):
    ...
    if row:
        ...
        for d in deliveries:
            cursor.execute("SELECT * FROM clientes WHERE client_id = ?", (d[1], ))
            c =cursor.fetchone()
...
def login():
    ...
    if user:
        ...
        email= simpledialog.askstring("New Freelacer", "Email: ")
        freelancer= Freelancer(user_id=100 +fid, name=name, email=email, freelacer_id= fid)
...
root=tk.Tk()
root.title("Freelancer Delivery Manager")
